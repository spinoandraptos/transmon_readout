...
day ="2025-07-13"
start_time = datetime.strptime("08-50-00", "%H-%M-%S")
end_time   = datetime.strptime("11-43-00", "%H-%M-%S")
...
if __name__ == "__main__":
    ...
    chunk_size = 30
    for i, (qubit, pulse) in enumerate(group_labels):
        photon_list = []
        group_filenames = sorted_filenames[i * chunk_size: (i + 1) * chunk_size]

        for filename in group_filenames:
            try:
                full_path = os.path.join(data_directory, filename)
                n = extract_residual_photons(full_path)
        ...
        try:
            popt, pcov = curve_fit(photon_decay, tlist / 1e9, photon_list,
                                p0=[photon_list[0], (tlist[-1] - tlist[0]) / 1e9 / 2])
            n0_fit, tau_fit = popt
            kappa_fit = 1 / tau_fit
        except Exception as e:
            print(f"Fit failed for {qubit} {pulse}: {e}")
            continue
        ...
        plt.title(f"{qubit.upper()} {pulse.upper()} - Photon Decay")
        plt.grid(True)
        plt.legend()
        plt.tight_layout()

        fig_path = os.path.join(CLEAR_directory, f"{qubit}_{pulse}_n0.png")
